let app = new Vue({
  ...
  methods: {
    initDataBits: function () {
      ...
      // Guard against invalid values from the number input
      if (
        !Number.isInteger(this.numberOfDataBits) ||
        this.numberOfDataBits < 1
      ) {
        return;
      }
    ...
    send: function () {
      if (this.validate(this.dataBits) === true) {
        ...
        return axios
          .put('http://localhost:3001/message', { bits: this.encodedMessage })
